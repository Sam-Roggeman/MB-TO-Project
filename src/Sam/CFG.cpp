...
CFG::CFG(std::vector<std::string> V, std::vector<std::string> T,
         std::map<std::string, std::vector<std::vector<std::string>>> P, std::string S):V(std::move(V)), T(std::move(T)), S(std::move(S)), P(std::move(P)) {
    this->sort();
}
...
void CFG::EliminateEpsilon() {
    ...
    while (found_nullable){
        ...
        for (auto& production_list : this->P){
            ...
            for (int index = 0; index < production_list.second.size(); index++){
                ...
                if (production_list.second[index].empty()){
                    ...
                    if(std::find(all_nullables.begin(), all_nullables.end(), production_list.first) == all_nullables.end()){
                        found_nullable = true;
        ...
        for (auto& production_list : this->P){
            ...
            for (int index = 0; index < production_list.second.size(); index++){
                ...
                for (auto symbol_ind = 0; symbol_ind != production_list.second[index].size(); symbol_ind++){
    ...
    this->sort();
    std::cout << "\n\n >> Eliminating epsilon productions\n  Nullables are " << all_nullables << "\n  Created "<< p_count_e <<" productions, original had "<<p_count_s<< "\n\n";
        ...
        for (auto& [from, productions] : this->P){
            ...
            for (auto& production : productions){
                if (this->isUnitproduction(production)){
                    ...
                    if (std::find(unit_pairs.begin(), unit_pairs.end(), std::make_pair(from, production[0])) == unit_pairs.end()){
            ...
            for (auto & production : productions){
                ...
                if (to_it != unit_productions.end() ){
                    for (auto& symbol : to_it->second){
                        ...
                        if (std::find(unit_pairs.begin(), unit_pairs.end(), std::make_pair(from, symbol)) == unit_pairs.end()){
...
void CFG::EliminateUselessSymbols() {
    ...
    for (const auto& sym:generating_symbols){
        non_generating_symbols.erase(std::find(non_generating_symbols.begin(), non_generating_symbols.end(), sym));
    }
    ...
    std::cout << "  Removed "<< std::get<0>(var_ter_prod) - this->V.size()<<" variables, "<<std::get<1>(var_ter_prod)-this->T.size()<<" terminals and " <<std::get<2>(var_ter_prod)-this->countProductions()<<" productions" <<"\n\n";
}
...
std::vector<std::string> CFG::findGeneratingSymbols() {
    ...
    while (!done){
        ...
        for (auto& [from, productions] : this->P){
            if (std::find(generating_symbols.begin(), generating_symbols.end(), from) == generating_symbols.end()){
                //loop over alle producties van de cfg, indien het een genererende productie heeft is het symbool genererend
                for (std::vector<std::string>& production:productions){
                    generating_flag = true;
                    for (std::string& symbol:production){
                        //indien een van de symbolen van de productie geen genererend symbool is, is de productie dit ook niet
                        if (std::find(generating_symbols.begin(), generating_symbols.end(), symbol) == generating_symbols.end()){
    ...
    while (!to_do.empty()){
        ...
        for (std::vector<std::string>& productions : this->P[current_symbol]){
            ...
            for (std::string & sym : productions ){
                ...
                if(std::find(reachable_symbols.begin(), reachable_symbols.end(), sym) == reachable_symbols.end()){
...
void CFG::replaceBadies() {
    ...
    for (auto& [from, producties]:this->P){
        ...
        for (auto& productie : producties){
            ...
            if (productie.size() == 1 && std::find(this->T.begin(), this->T.end(), productie[0]) != this->T.end()){
    ...
    for (auto& [from, producties]:this->P){
        for (auto& productie : producties){
            for (auto& symbol:productie){
                if (std::find(this->T.begin(), this->T.end(), symbol) != this->T.end() &&terminal_to_var.at(symbol)!=from ){
    ...
    std::cout << " >> Replacing terminals in bad bodies\n"
                 "  Added " << this->V.size() - start_var <<" new variables: "<<new_var<<"\n  Created " <<
        ...
        for (auto& [from, productions]:this->P){
            for (auto& production : productions){
                ...
                if (production.size()>2){
                    ...
                    //nieuwe productie vanuit de nieuwe variabele tot de laatste 2 symbolen van de productie
                    this->P[new_var] = {{production.at(production.size() - 2), production.at(production.size() - 1)}};
...
const std::map<std::string, std::vector<std::vector<std::string>>> &CFG::getP() const {
    return P;
}

const std::vector<std::string> &CFG::getV() const {
    return V;
}

const std::vector<std::string> &CFG::getT() const {
    return T;
}
