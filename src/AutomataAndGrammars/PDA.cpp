//
// Created by AutomataAndGrammars Roggeman on 29/10/2021.
//
        ...
        for (auto& transition : this->Transitions) {
                ...
                if (stac_rep_count != 0) {
                        ...
                        for (std::vector<std::string>& to_states : combinationsOfStates(stac_rep_count)) {
                                ...
                                for (auto to_state_it = to_states.begin() + 1; to_state_it != to_states.end();
                                     to_state_it++) {
                                        // voeg het stacksymbool toe en de volgende state: X, p][p,
                                        to += std::get<4>(transition)[n] + "," + (*to_state_it) + "] [" +
                                              (*to_state_it) + ",";
                                        n++;
                                }
                                ...
                                val = {to};
                                P["[" + std::get<0>(transition) + "," + std::get<2>(transition) + "," + to_states[0] +
                                  "]"]
                                    .emplace_back(val);
                        }
                }
                // nul stacksymbolen bij de transitie is een speciaal geval
                        ...
                        P["[" + std::get<0>(transition) + "," + std::get<2>(transition) + "," +
...
std::vector<std::vector<std::string>> PDA::combinationsOfStates(
    int n_stacksymbols, std::vector<std::vector<std::string>> list_to_iterate) const
{
